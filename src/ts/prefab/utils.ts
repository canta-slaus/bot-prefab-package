...
class PrefabUtils {
    ...
    async getReply (message: Message, options?: { time?: number, user?: User, words?: string[], regexp?: RegExp }): Promise<Message | undefined> {
    	let time = 30000;
    ...
    async getCooldown (command: Command, message: Message|CommandInteraction) {
    	...
    	if (message.guildId) {
    		...
    		if (guildInfo.prefab.commandCooldowns && guildInfo.prefab.commandCooldowns[command.name]) {
    			...
    			// @ts-ignore
    			const highestRole = message.member.roles.cache.filter(role => roles.includes(role.id)).sort((a, b) => b.position - a.position).first();
